...
#### 1. Я відразу не попала на стартову сторінку тому переглянула її. Там описано як налаштувати інтеграцію з `GitHub` та додати мій репозиторій;
#### 2. Основна документація по `Travis CI` представлена на сторінці.
#### 3. У лабораторній 6 я буду налаштовувати CI/CD сервер. Інтеграція пройшла успішно тому на дашборі `Travis` відображаються мої `GitHub` репозиторії. Додала репозиторій до `Travis`.
#### 4. Для того щоб `Travis` знав які кроки потрібно виконати над моїм кодом у кореневій папці мого репозиторію створюю файл `.travis.yml`. Створила у моєму `GitHub` репозиторію такий самий файл та скопіювала туди вміст. Travis автоматично знайшов даний файл та виконав ці кроки при кожному новому коміті в `master` гілку.
  ...
  include:
    ...
    - stage: "Build Lab 4."
      ...
      install:
        - cd ./Lab4/
      script:
        - docker build -t ivanova/lab4:django-travis .
        - docker images
        - if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin; docker push ivanova/lab4:django-travis; else echo "PR skip deploy"; fi
    - stage: "Build Lab 5."
...
exit 0
```
Оскільки сервер на `Django` для 3 лабораторної запустився і не вимикався я примусово зупинила білд для цього під-тесту. Всі інші під-тести пройшли успішно. 
...
```text
    - stage: "Build Lab 4."
      ...
      install:
        - cd ./Lab4/
      script:
        - docker build -f Dockerfile -t ivanova/lab4:django-travis .
        - docker build -f Dockerfile.site -t ivanova/lab4:monitoring-travis .
        - docker images
        - if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin; docker push ivanova/lab4:django-travis; docker push ivanova/lab4:monitoring-travis; else echo "PR skip deploy"; fi
```
...
```text
    - stage: "Build Lab 5."
      ...
      script:
        ...
        - make docker-push
```
* ##### Посилання на мій [***Travis***](https://app.travis-ci.com/github/AnhelinaIvanova/Ivanova_IK_31) білд.
